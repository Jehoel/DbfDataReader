using System;
using Shouldly;
using Xunit;

namespace Dbf.Tests
{
    [Collection("dbase_30")]
    public class Dbase30Tests : DbaseTests
    {
        private const string Dbase30FixturePath = "dbase_30.dbf";

        public Dbase30Tests()
            : base( Dbase30FixturePath )
        {
        }

        [Fact]
        public void Should_report_correct_record_count()
        {
            this.dbfTable.Header.RecordCount.ShouldBe(34);
        }

        [Fact]
        public void Should_report_correct_version_number()
        {
            this.dbfTable.Header.Version.ShouldBe( (Byte)0x30 );
        }

        [Fact]
        public void Should_report_that_the_file_is_not_foxpro()
        {
            this.dbfTable.Header.IsFoxPro.ShouldBeTrue();
        }

        [Fact]
        public void Should_have_the_correct_number_of_columns()
        {
            this.dbfTable.Columns.Count.ShouldBe(145);
        }

        [Fact]
        public void Should_have_the_correct_column_schema()
        {
            ValidateColumnSchema("dbase_30_summary.txt");
        }


        [Fact]
        public void Should_have_correct_row_values()
        {
            ValidateRowValues( "dbase_30.csv", trimTextFromCsvFile: true );
        }

        [Fact]
        public void Should_have_correct_row_values_for_subsets()
        {
            ValidateRowValuesSubset( "dbase_30.csv", trimTextFromCsvFile: true );
        }
    }
}